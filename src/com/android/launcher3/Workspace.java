...
import com.android.launcher3.snail.custompage.CustomPage;
...
public class Workspace extends PagedView
    ...
    public void bindAndInitFirstWorkspaceScreen(View qsb) {
        ...
        if (FeatureFlags.HIDE_QSB) { // add by linmq 2018-6-27
            return;
        }
    ...
    public CellLayout insertNewWorkspaceScreen(long screenId, int insertIndex) {
        ...
        //add by linmq 2018-6-26 add custom page
        if (FeatureFlags.ADD_CUSTOM_PAGE && insertIndex == CUSTOM_PAGE_SCREEN_POS) {
            if (!hasCustomContent()) {
                return createCustomContentPage();
            }
        }
    ...
    private CellLayout createCustomContentPage() {
        long screenId = -1;
        int insertIndex = getChildCount();
        if (mWorkspaceScreens.containsKey(screenId)) {
            throw new RuntimeException("Screen id 456" + screenId
                    + " already exists!");
        }

        CellLayout newScreen = (CellLayout)
                mLauncher.getLayoutInflater().inflate(R.layout.snail_custom_page, null);
        newScreen.setOnLongClickListener(mLongClickListener);
        newScreen.setOnClickListener(mLauncher);
        newScreen.setSoundEffectsEnabled(false);
        mWorkspaceScreens.put(screenId, newScreen);
    ...
    public long commitExtraEmptyScreen() {
        ...
        int index = getPageIndexForScreenId(EXTRA_EMPTY_SCREEN_ID); // add by linmq 2016-8-6-26
        ...
        // Update the page indicator marker  add by linmq 2018-6-25
        if (FeatureFlags.SNAIL_PAGE_INDICATOR && getPageIndicator() != null) {
    ...
    public void stripEmptyScreens() {
        ...
        for (int i = 0; i < total; i++) {
            if (FeatureFlags.ADD_OR_DELETE_CELLLAYOUT) { // add by linmq 2018-6-26
                break;
            }
    ...
    public boolean checkAddOrDeleteEmptyLayout() {
        if (isInOverviewMode()) {
            setAddCellLayoutFlag(true);
            addOrDeleteEmptyLayout(Workspace.State.OVERVIEW);
            return true;
        }
        return false;
    }
   ...
   void mapPointFromChildToSelf(View v, float[] xy) {
    ...
    public void onDragOver(DragObject d) {
        ...
        if (mDragTargetLayout != null) {
            //add by linmq 2017-3-15
            if (FeatureFlags.ADD_CUSTOM_PAGE && mDragTargetLayout instanceof CustomPage) {
                return;
            }
            // We want the point to be mapped to the dragTarget.
            if (mLauncher.isHotseatLayout(mDragTargetLayout)) {
    ...
    // add begin by linmq 2017-3-8
    @Override
    protected void dispatchDraw(Canvas canvas) {
        super.dispatchDraw(canvas);
        if (FeatureFlags.SCROLL_LOOP && !(mState == State.OVERVIEW)) {
            ...
            if (!fastDraw) {
                ...
                if (scrollPos != leftScreen && isScreenNoValid(rightScreen)) {
                    if (rightScreen == 0 && isScrollToRight) { // // 最后一页 》》》 第0页（向右滚动）
                        int offset = childCount * width;
                        canvas.translate(+offset, 0);
    ...
    private final static long EXTRA_ADD_SCREEN_ID = -401;
    public final static int CUSTOM_PAGE_SCREEN_POS = 0;
    public final static long CUSTOM_PAGE_SCREEN_ID = 0L;
    ...
    public void addOrDeleteEmptyLayout(final State state) {
        if (FeatureFlags.ADD_OR_DELETE_CELLLAYOUT) {
    ...
    private void setEmptyScreenDeleteIcon(final State state) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            // modify by linmq 2018-6-26 自定义页不需要画删除图标
            CellLayout cl = (CellLayout) child;
    ...
    public long addCreatorScreen() {
        ...
        CellLayout newScreen = (CellLayoutCreator) mLauncher
                .getLayoutInflater().inflate(R.layout.workspace_screen_creator, null);
        newScreen.setOnClickListener(mLauncher);
        newScreen.setBackgroundAlpha(1f);
        newScreen.setSoundEffectsEnabled(false);
    ...
    public long addNewScreen() {
        ...
        if (mWorkspaceScreens.containsKey(newId)) {
            throw new RuntimeException("Screen id " + newId
                    + " already exists!");
        }
        CellLayout newScreen = (CellLayout) mLauncher.getLayoutInflater()
        ...
        addView(newScreen, insertIndex);
        newScreen.setBackgroundAlpha(1f); // 设置背景透明度
