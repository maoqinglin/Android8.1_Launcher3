...
import com.android.launcher3.pageindicators.SnailPageIndicator;
...
public abstract class PagedView extends ViewGroup implements ViewGroup.OnHierarchyChangeListener {
    ...
    protected static final int INVALID_PAGE = FeatureFlags.SCROLL_LOOP ? -100 : -1;
    ...
    protected void onAttachedToWindow() {
        ...
        if (FeatureFlags.SNAIL_PAGE_INDICATOR) {
            initSnailPageIndicator();
        }
    }
    ...
    // add by linmq 2018-6-25
    public void initSnailPageIndicator(){
    ...
    protected void updateCurrentPageScroll() {
        // ddd by linmq 2018-6-21
        if (FeatureFlags.SCROLL_LOOP) {
    ...
    private int validateNewPage(int newPage) {
        // add by linmq 2018-6-21
        if (FeatureFlags.SCROLL_LOOP && !(this instanceof FolderPagedView)) {
            return Math.max(-1, Math.min(newPage, getChildCount())); // [-1,getChildCount()]
        }
    ...
    protected boolean computeScrollHelper(boolean shouldInvalidate) {
        if (mScroller.computeScrollOffset()) {
            ...
            return true;
        } else if (mNextPage != INVALID_PAGE || (FeatureFlags.SCROLL_LOOP && mCurrentPage == INVALID_PAGE)) {
            sendScrollAccessibilityEvent();
            // add by linmq 2018-6-21 首尾切换时，最后需要划动的距离
            if(FeatureFlags.SCROLL_LOOP){
                int pageCount = getPageCount();
                if (pageCount != 0) {
                    mCurrentPage = Math.max(0, Math.min((mNextPage + pageCount) % pageCount, pageCount - 1));
                }
                final int newX = getScrollForPage(mCurrentPage);
                scrollTo(newX, getScrollY());
            }
            int prevPage = mCurrentPage;
            if (!FeatureFlags.SCROLL_LOOP) { // modify by linmq 2018-6-21
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        //add by linmq 2017-3-8
        if (FeatureFlags.SCROLL_LOOP && needUpdateFreeScrollBound) {
    ...
    public void onChildViewAdded(View parent, View child) {
        ...
        if (mPageIndicator != null && !isReordering(false)) {
            if (FeatureFlags.SNAIL_PAGE_INDICATOR) { // modify by linmq 2018-6-25
                int pageIndex = indexOfChild(child);
                mPageIndicator.addMarker(pageIndex, getPageIndicatorMarker(pageIndex), true);
            } else {
    ...
    public void removeView(View v) {
        ...
        removeMarkerForView(indexOfChild(v)); // modify by linmq 2018-6-25
        super.removeView(v);
    }
    @Override
    ...
    public void removeViewAt(int index) {
        // XXX: We should find a better way to hook into this before the view
        // gets removed form its parent...
        removeMarkerForView(index); // modify by linmq 2018-6-25
     ...
     * @return
     */
    protected int getEdgePageOffset(int index) {
        int onePageOffset = getOnePageOffset();
        if (PRE_EDGE_PAGE == index) {
            return -onePageOffset;
        } else if (getChildCount() == index) {
            return getChildCount() * onePageOffset;
        }
        return onePageOffset;
    }
    ...
    public void requestDisallowInterceptTouchEvent(boolean disallowIntercept) {
        if (disallowIntercept) {
            ...
            if (currentPage != null) {  // add by linmq 2018-6-21
            ...
            case MotionEvent.ACTION_DOWN: {
                ...
                if (finishedScrolling) {
                    ...
                    if (!mScroller.isFinished() && !mFreeScroll) {
                        if (!FeatureFlags.SCROLL_LOOP) {
                            setCurrentPage(getNextPage());//add by linmq 2018-6-21 修复循环滑动时，动画未停止导致滑动不流畅问题
                        }
                        //add by linmq 2018-6-21 立即停止滑动
                        if (FeatureFlags.SCROLL_LOOP) {
                            mScroller.abortAnimation();
                        }
                        pageEndTransition();
    ...
    protected float getScrollProgress(int screenCenter, View v, int page) {
        ...
        if (FeatureFlags.SCROLL_LOOP) {  // add by linmq 2018-6-21
            if (mCurrentPage == getChildCount() - 1 && page == 0 && getScrollX() > 0) {
                screenCenter -= getChildCount() * getViewportWidth();
            } else if (mCurrentPage == 0 && page == getChildCount() - 1 && getScrollX() < 0) {
                screenCenter += getChildCount() * getViewportWidth();
            }
        }
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        case MotionEvent.ACTION_UP:
            if (mTouchState == TOUCH_STATE_SCROLLING) {
                ...
                // modify by linmq 2018-6-21
                final int pageWidth = FeatureFlags.SCROLL_LOOP
                        ? getPageAt(Math.max(0, Math.min(mCurrentPage, getPageCount() - 1))).getMeasuredWidth()
                        : getPageAt(mCurrentPage).getMeasuredWidth();
                ...
                if (!mFreeScroll) {
                    ...
                    if (((isSignificantMove && !isDeltaXLeft && !isFling) ||    // 向右移动，滑动到上一页；主要判断距离
                            (isFling && !isVelocityXLeft)) &&   // 向右快速滑动距离大于阈值；主要判断速度
                            (FeatureFlags.SCROLL_LOOP ? (mCurrentPage >= 0) : (mCurrentPage > 0))) { // 第0页继续右滑,进入最后一页，实现循环滑动
                        finalPage = returnToOriginalPage ? mCurrentPage : mCurrentPage - 1; // 滑动到上一页
                        snapToPageWithVelocity(finalPage, velocityX);
                    } else if (((isSignificantMove && isDeltaXLeft && !isFling) || // 向左移动距离大于宽度阈值
                            (isFling && isVelocityXLeft)) && // 向左快速滑动距离大于阈值
                            (FeatureFlags.SCROLL_LOOP ? (mCurrentPage < getChildCount()): (mCurrentPage < (getChildCount() - 1)))) { // 在最后一页继续左滑，进入第0页,实现循环滑动
                        finalPage = returnToOriginalPage ? mCurrentPage : mCurrentPage + 1; // 滑到下一页
                        snapToPageWithVelocity(finalPage, velocityX);
                    } else {
                        // modify by linmq 2015-8-6-21
                        if (FeatureFlags.SCROLL_LOOP) {
                            snapToPageWithVelocity(mCurrentPage, velocityX); // 恢复滑动前的状态
                        } else {
                            snapToDestination();
                        }
                    }
    ...
    public void scrollLeft() {
        // modify by linmq 2018-6-22
        if (getNextPage() > 0 || (FeatureFlags.SCROLL_LOOP && getNextPage() == 0))
            snapToPage(getNextPage() - 1);
    }

    public void scrollRight() {
        // modify by linmq 2018-6-22
        if (getNextPage() < getChildCount() - 1 || (FeatureFlags.SCROLL_LOOP && getNextPage() == getChildCount() - 1))
            snapToPage(getNextPage() + 1);
    }
