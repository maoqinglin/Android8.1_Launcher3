...
public class DeviceProfile {
    ...
    private void adjustToHideWorkspaceLabels() {
        ...
        if (!FeatureFlags.LANDSCAPE_EXTEND) { // add by linmq 2018-7-7 修复横屏不显示应用名称问题
            iconTextSizePx = 0;
                ...
                if (FeatureFlags.SCROLL_LOOP) {
                    padding.set(0, 0,
                            hotseatBarSizePx + hotseatBarRightNavBarRightPaddingPx
                                    + hotseatBarRightNavBarLeftPaddingPx, edgeMarginPx);
                } else {
    ...
    private int getWorkspacePageSpacing() {
        if (isVerticalBarLayout() || isLargeTablet) {
            ...
            // add by linmq 2018-7-6 横屏页面间距
            if (FeatureFlags.SCROLL_LOOP) {
                return getWorkspacePadding(null).right;
            }
            ...
            if(FeatureFlags.SCROLL_LOOP){
                return Math.max(defaultPageSpacingPx, getWorkspacePadding(null).left * 2);
            }
