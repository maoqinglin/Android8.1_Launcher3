...
public class LoaderTask implements Runnable {
                        ...
                        case LauncherSettings.Favorites.ITEM_TYPE_DEEP_SHORTCUT:
                            ...
                            if (!TextUtils.isEmpty(targetPkg) && !validTarget) {
                                ...
                                if (c.restoreFlag != 0) {
                                    ...
                                    //modify by linmq 2018-7-23 不显示冻结应用时才移除
                                    if (FeatureFlags.SHOW_DISABLE_ICON) {
                                        allowMissingTarget = true; // 避免桌面意外停止运行，导致数据库中被冻结的应用无法加载
                                    } else {
    ...
    private void verifyApplications() {
        HashSet<ItemInfo> tmpInfos;
        List<Pair<ItemInfo, Object>> added = new ArrayList<>();
        HashSet<ComponentName> compSet = new HashSet<>();
        synchronized (mBgAllAppsList) {
            for (AppInfo app : mBgAllAppsList.data) {
                compSet.clear();
                compSet.add(app.componentName);
                tmpInfos = mBgDataModel.getItemInfoForComponentName(compSet, app.user); // 将所有应用中桌面不存在的应用加载到桌面
                if (tmpInfos.isEmpty()) {
                    // We are missing an application icon, so add this to the workspace
                    added.add(Pair.create((ItemInfo) app, null));
        ...
        if (!added.isEmpty()) {
            mApp.getModel().addAndBindAddedWorkspaceItems(Provider.of(added));
        }
    }
}
