...
public class AddWorkspaceItemsTask extends BaseModelUpdateTask {
    ...
    protected Pair<Long, int[]> findSpaceForItem(
            ...
            int spanX, int spanY) {
        LongSparseArray<ArrayList<ItemInfo>> screenItems = new LongSparseArray<>(); // {screenid : items} 二维映射表
        ...
        if (preferredScreenIndex < screenCount) { // 如果有多页，优先从screen = 1 页开始找
        ...
        if (!found) {
            ...
            for (int screen = 0; screen < screenCount; screen++) { // modify by linmq 2018-7-19 总是从screen = 0 页开始找位置
    ...
    private boolean findNextAvailableIconSpaceInScreen(
        ...
        GridOccupancy occupied = new GridOccupancy(profile.numColumns, profile.numRows); // 二维棋盘
        if (occupiedPos != null) {
            for (ItemInfo r : occupiedPos) {
                occupied.markCells(r, true); // 设置应用占坑
            }
        }
