    ...
    public void execute(LauncherAppState app, BgDataModel dataModel, AllAppsList appsList) {
        ...
        if (FeatureFlags.SHOW_DISABLE_ICON) {
            if (mOp == OP_ADD || mOp == OP_UPDATE) {
                FrozenEx.reLoadedLauncherApp(context);
            }
        }
        ...
        switch (mOp) {
            ...
            case OP_UPDATE:
                for (int i = 0; i < N; i++) {
                    ...
                    if (FeatureFlags.SHOW_DISABLE_ICON && FrozenEx.isAppEnableSetting(context, packages[i])) {
                        iconCache.updateIconsForPkg(packages[i], mUser);
        ...
        if (!addedOrModified.isEmpty()) {
            List<Pair<ItemInfo,Object>> addedList = new ArrayList<>();
            for (AppInfo ai : addedOrModified) {
                addedOrUpdatedApps.put(ai.componentName, ai);
                addedList.add(Pair.create((ItemInfo) ai,null));
            }
            if (FeatureFlags.DISABLE_ALL_APPS) {
                app.getModel().addAndBindAddedWorkspaceItems(Provider.of(addedList));
            }
            scheduleCallbackTask(new CallbackTask() {
                @Override
                public void execute(Callbacks callbacks) {
                    callbacks.bindAppsAddedOrUpdated(addedOrModified); // 由于显示双层搜索功能，应用更新时，需要更新搜索的应用列表
                }
            });
        }
        ...
        if (mOp == OP_REMOVE) {
            ...
            for (int i=0; i<N; i++) {
                if (!launcherApps.isPackageEnabledForProfile(packages[i], mUser)) {
                    // modify by linmq 2018-7-23  不删除禁用的app图标
                    if (!FeatureFlags.SHOW_DISABLE_ICON) {
