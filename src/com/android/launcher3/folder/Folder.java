...
import com.android.launcher3.snail.folder.ImageHelper;
import com.android.launcher3.snail.util.ScreenCapture;
...
public class Folder extends AbstractFloatingView implements DragSource, View.OnClickListener,
    ...
    protected void onFinishInflate() {
        ...
        //add by linmq 2018-6-28
        mFolderName.measure(measureSpec, measureSpec); // 显示文件夹名称
        mFolderNameHeight = mFolderName.getMeasuredHeight();
        mFolderFrame = findViewById(R.id.folder_frame);
    }
    ...
    static Folder fromXml(Launcher launcher) {
        if (FeatureFlags.CUSTOM_FOLDER) {
            return (Folder) launcher.getLayoutInflater().inflate(R.layout.snail_user_folder, null);
        }
    ...
    @Thunk void onDragOver(DragObject d, int reorderDelay) {
        ...
        // add by linmq 2018-6-28
        if (FeatureFlags.CUSTOM_FOLDER) {
            if (!isPointInFolderFrame(false,(int) r[0], (int) r[1])) { // 使用相对Frame的坐标
                close(true);
            }
        }
    }
    ...
    private void centerAboutIcon() {
        ...
        int width = getFolderWidth();
        int height = getFolderHeight();
        if (FeatureFlags.CUSTOM_FOLDER) {  //add by linmq 2018-6-28
        ...
        if (FeatureFlags.CUSTOM_FOLDER) { // add by linmq 2018-6-28
            ...
            int frameWidth = mFolderFrame.getPaddingLeft() + mFolderFrame.getPaddingRight() + mContent.getDesiredWidth();
            int frameHeigth = mFolderFrame.getPaddingTop() + mFolderFrame.getPaddingBottom()
                    + mContent.getDesiredHeight() + mFolderNameHeight + mFooterHeight;
            Rect frame = new Rect();
            mLauncher.getWindow().getDecorView().getWindowVisibleDisplayFrame(frame);
            int mStatusBarHeigth = frame.top;
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        if (FeatureFlags.CUSTOM_FOLDER) {
            ...
            int frameHeight = mFolderFrame.getPaddingTop() + mFolderFrame.getPaddingBottom() + mContent.getDesiredHeight()
                    + mFolderNameHeight + mFooterHeight;

            mFolderFrame.measure(MeasureSpec.makeMeasureSpec(frameWidth,
                    MeasureSpec.EXACTLY), MeasureSpec.makeMeasureSpec(frameHeight,
                    MeasureSpec.EXACTLY));
            mFolderName.measure(MeasureSpec.makeMeasureSpec(mFolderName.getMeasuredWidth(),
                    MeasureSpec.EXACTLY),
        ...
        // add by linmq 2018-6-28
        if (FeatureFlags.CUSTOM_FOLDER) {
            int width = mLauncher.getWindow().getDecorView().getWidth();
            int height = mLauncher.getWindow().getDecorView().getHeight();
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        if (FeatureFlags.CUSTOM_FOLDER) {
            ...
            if (!isPointInFolderFrame(true,(int) touchX, (int) touchY)) {
                close(true);
            }
            return true;
        }
    ...
    private boolean isPointInFolderFrame(boolean isRelativeFolder,int x, int y) {
        int transLeft = isRelativeFolder ? mFolderFrame.getLeft() : 0; // 文件夹内点击相对于文件夹的坐标，相当于绝对坐标
        int transTop = isRelativeFolder ? mFolderFrame.getTop() : 0; // 文件夹内拖动应用相对的是Frame坐标
        if (x < transLeft || x > transLeft + mFolderFrame.getWidth()
                || y < transTop || y > transTop + mFolderFrame.getHeight()) {
            return false;
        }
        return true;
    }
    ...
    public void applyBlur() {
        getViewTreeObserver().addOnPreDrawListener(new ViewTreeObserver.OnPreDrawListener() {
            ...
            public boolean onPreDraw() {
                ...
                ImageHelper.blur(context, Folder.this, screenBmp);
                return true;
            }
        });
    }
