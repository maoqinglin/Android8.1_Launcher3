...
public class FolderPagedView extends PagedView {
    ...
    public FolderPagedView(Context context, AttributeSet attrs) {
        ...
        //add by linmq 2018-6-28 文件夹横屏行数处理
        if (FeatureFlags.CUSTOM_FOLDER && context.getResources().getConfiguration().orientation == Configuration.ORIENTATION_LANDSCAPE) {
            mMaxCountY = 3;
        }
    ...
    public void setupContentDimensions(int count) {
        ...
        //add by linmq 2018-6-28 文件夹四列，每次从左至又依次排列
        if (FeatureFlags.CUSTOM_FOLDER) {
            mGridCountX = mMaxCountX;
            mGridCountY = mMaxCountY; //add by linmq 2018-6-28 文件夹横屏显示行数
        }
        // Update grid size
