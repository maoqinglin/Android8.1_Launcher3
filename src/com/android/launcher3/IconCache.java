...
public class IconCache {
    ...
    public Drawable getCustomIcon(ComponentName component) {
        Bitmap bitmap = LauncherAppState.getInstance(mContext).getIconDecorater().decorateIcon(component);
        if (bitmap != null) {
    ...
    public Drawable getFullResIcon(LauncherActivityInfo info, boolean flattenDrawable) {
        if (FeatureFlags.CUSTOM_APP_ICON) { // add by linmq 2018-7-17 替换系统应用图标
            Drawable drawable = getCustomIcon(info.getComponentName());
            if (drawable != null) {
                return drawable;
            }
        }
    ...
    @Thunk synchronized void addIconToDBAndMemCache(LauncherActivityInfo app,
        ...
        if (entry == null) {
            ...
            entry.icon = LauncherIcons.createBadgedIconBitmap(getFullResIcon(app), app.getUser(),
                    mContext,  Build.VERSION_CODES.O); // modify by linmq 2018-7-18 使图标可以做自适应处理
        }
            ...
            if (!getEntryFromDB(cacheKey, entry, useLowResIcon)) {
                try {
                    ...
                    if (FeatureFlags.CUSTOM_APP_ICON) {
                        Drawable drawable = getCustomIcon(cn);
                        if (drawable != null) {
                            appDrawable = drawable;
                        }
                    }
                    if(appDrawable == null){
                        appDrawable = appInfo.loadIcon(mPackageManager);
                    }
                    Bitmap icon = LauncherIcons.createBadgedIconBitmap(
                            appDrawable, user, mContext, Build.VERSION_CODES.O);
