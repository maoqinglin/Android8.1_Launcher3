...
import com.android.launcher3.snail.appicon.LoadThemeUtil.ThemeApp;
...
public class IconDecorater {
    ...
    public IconDecorater(Context context) {
        ...
        Log.i("lmq","mThemeApps.size = "+mThemeApps.size());
    ...
    public Bitmap decorateIcon(ResolveInfo info) {
        ...
        if (icon == null) {
            if (FeatureFlags.CUSTOM_APP_ICON) { // add by linmq 2018--7-17
                icon = drawableToBitmap(getFullResIcon(info));
            } else {
                icon = LauncherIcons.createIconBitmap(icon, mContext);
            }
            icon = decorateIcon(mContext.getResources(), icon);
        }
    ...
    // add by linmaoqing 2017-3-14
    public Bitmap decorateIcon(ComponentName component) {
        Bitmap icon = null;
        ThemeApp app = new ThemeApp(component.getPackageName(),
		...
		/*if (icon == null) {
			...
			try {
			    //add by linmq 2017-11-24
                info = mPackageManager.getActivityInfo(component, FeatureFlags.SHOW_DISABLE_ICON? PackageManager.MATCH_DISABLED_COMPONENTS : 0);
            } catch (NameNotFoundException e) {
				e.printStackTrace();
				info = null;
			}
			if (FeatureFlags.CUSTOM_APP_ICON) {
				icon = drawableToBitmap(getFullResIcon(info));
			} else {
    ...
    public Bitmap observeBadgeIcon(final TextView textView, Bitmap icon, ComponentName componentName) {
        String className = componentName == null ? "" : componentName.getClassName();
        if (!canIconBeObserved(className)) {
            return icon;
        }
        ...
        synchronized (mObservedTextViews) {
            ...
            mObservedTextViews.put(className, new WeakReference<TextView>(textView));
            mObservedIcons.put(className, icon);
            return decorateIconNum(icon, className);
        }
    }
    	...
    	synchronized (mObservedTextViews) {
            ...
            if (wTextView != null) {
                ...
                final Bitmap mask = markCountIcon(mContext.getResources(),
                        ...
                        unreadNum);
                //modify by linmq 2017-7-4 fix badge icon change small
                if (textView instanceof BubbleTextView) {
//                    ((BubbleTextView) textView).updateIcon(mask);
                }
            }
        }
    }
...
/*        Bitmap maskedIcon = maskIcon(mIconMask.getWidth(), mIconMask.getHeight(),
    ...
    private Bitmap markCountIcon(Resources resources, Bitmap icon, int count) {
        ...
        Bitmap indicate = BitmapFactory.decodeResource(resources, R.drawable.snail_new_number_bg);
...
//        mIconFlag = SnailConfig.getInstance().isLauncherShortcutNeedBg();
    }
}
