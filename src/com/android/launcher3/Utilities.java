...
public final class Utilities {
    ...
    public static void mapCoordInSelfToDescendant(View descendant, View root, int[] coord) {
        sMatrix.reset();
        View v = descendant;
        if (FeatureFlags.CUSTOM_FOLDER && v instanceof Folder) { // add by linmq 2018-6-28  修改父view Folder为mFolderFrame
            v = ((Folder) v).getChildAt(0); // 设置长按文件夹中应用排序时相对的父view，默认本来是Folder；但是自定义后的Folder为全屏，转换坐标无效
            if (v == null) {
                Log.e(TAG, "mapCoordInSelfToDescendant can not find Folder frame");
                return;
            }
        }
        while(v != root) {
    ...
    public static boolean arePredictiveAppsEnabled(Context context) {
        ...
        return prefs.getBoolean(SettingsActivity.KEY_PREDICTIVE_APPS, true);
    }
