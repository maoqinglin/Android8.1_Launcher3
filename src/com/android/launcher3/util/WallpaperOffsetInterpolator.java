...
public class WallpaperOffsetInterpolator implements Choreographer.FrameCallback {
    ...
    private float flushWallpaperOffset() {
        ...
        float scrollProgress = mWorkspace.getScrollX() / (float) scrollRange;
        if (scrollProgress > 1) {
            scrollProgress = -(float) (scrollProgress - 1 - 1.0f / (mWorkspace.getChildCount() - 1));
        } else if (scrollProgress < 0) {
            scrollProgress = (float) (1 - 1.0f / (mWorkspace.getChildCount() - 1) - scrollProgress);
        }
        return scrollProgress;
    }

    private int getScrollRange() {
        return mWorkspace.getChildOffset(mWorkspace.getChildCount() - 1) - mWorkspace.getChildOffset(0);
    }
