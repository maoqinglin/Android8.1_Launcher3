...
import com.android.launcher3.snail.celllayout.CellLayoutCreator;
...
public class Launcher extends BaseActivity
    ...
    private PredictiveAppsProvider mPredictiveAppsProvider; // add by linmq 2018-7-1-20
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mPredictiveAppsProvider = new PredictiveAppsProvider(this); // add by linmq 2018-7-20
        ...
        if (!mModel.startLoader(currentScreen)) {
            ...
            if (FeatureFlags.ADD_CUSTOM_PAGE && !mFinishedBindingItems) { // 没有完成item的绑定才调用，避免自定义页时序问题导致无法长按
    ...
    private void loadExtractedColorsAndColorItems() {
        ...
        if (Utilities.ATLEAST_NOUGAT) {
            ...
            if (!FeatureFlags.SNAIL_PAGE_INDICATOR && mWorkspace.getPageIndicator() != null) {
    ...
    protected void onResume() {
        ...
        if (!FeatureFlags.DISABLE_ALL_APPS && shouldShowDiscoveryBounce()) { // add by linmq 2018-7-7 禁止hotseat与allapp间跳动动画
    ...
    public boolean onKeyUp(int keyCode, KeyEvent event) {
        ...
        // add by linmq 2018-7-6 增加左右L1,R1键翻页
        if (FeatureFlags.KEY_EXTEND) {
            // 增加左右L1,R1键翻页
            switch (keyCode) {
                case FeatureFlags.KEYCODE_BUTTON_L1:
                    mWorkspace.scrollLeft();
                    mDragLayer.clearResizeFrame();
                    break;
                case FeatureFlags.KEYCODE_BUTTON_R1:
    ...
    public View createShortcut(ViewGroup parent, ShortcutInfo info) {
        BubbleTextView favorite = (BubbleTextView) LayoutInflater.from(parent == null ? mWorkspace.getContext() : parent.getContext()) // modify by linmq 2018-7-19
    ...
    public void onClick(View v) {
        ...
        // add by linmq 2018-6-26
        if (FeatureFlags.ADD_OR_DELETE_CELLLAYOUT && v instanceof CellLayoutCreator) {
            if (mWorkspace.checkAddOrDeleteEmptyLayout()) {
                return;
            }
        }
        ...
        if (v instanceof CellLayout) {
            if (mWorkspace.isInOverviewMode()) {
                ...
                if (FeatureFlags.ADD_OR_DELETE_CELLLAYOUT && layout.getEmptyScreenHelper().isClickDelete()) {//add by linmq 2018-6-26
                    return;
                }
    ...
    protected void onClickAppShortcut(final View v) {
        ...
        if (FeatureFlags.SHOW_DISABLE_ICON) { // && !Utilities.isSystemApp(this, shortcut.getIntent()) 不判断系统应用，第三方可能修改标示
            if (shortcut.intent.getComponent() != null) {
                FrozenEx.setAppState(this, shortcut.intent.getComponent().getPackageName());
            } else {
                FrozenEx.enableShortcut(this, shortcut.intent);
            }
            // add by linmq 2018-1-15 修复快捷方式解冻应用
            if (!FrozenEx.isQiHooApp(shortcut.intent) && FrozenEx.isLauncherApp(this, shortcut.intent)) {
                FrozenEx.updateLauncherApp(this);
            }
        }
        ...
        try {
            ...
            try {
                ...
                if (info.itemType == LauncherSettings.Favorites.ITEM_TYPE_DEEP_SHORTCUT) {
                    ...
                    if (isAllAppsVisible() || isWorkspaceAppVisible()) {
    ...
    // add by linmq 2018-7-22
    public boolean isWorkspaceAppVisible() {
        return FeatureFlags.DISABLE_ALL_APPS && mState == State.WORKSPACE;
    }
    ...
    public void showAppsView(boolean animated, boolean updatePredictedApps) {
        // add by linmq 2018-6-20 不显示全部应用
...
//            return;
    ...
    public void tryAndUpdatePredictedApps() {
        if (!mSharedPrefs.getBoolean(SettingsActivity.KEY_PREDICTIVE_APPS, true)) {
            mAppsView.setPredictedApps(new ArrayList<ComponentKeyMapper<AppInfo>>());
            return;
        }
    ...
    public void startBinding() {
        if (FeatureFlags.ADD_CUSTOM_PAGE) {
            mFinishedBindingItems = true;
        }
        if (LauncherAppState.PROFILE_STARTUP) {
    ...
    public void bindScreens(ArrayList<Long> orderedScreenIds) {
        ...
        if (FeatureFlags.ADD_CUSTOM_PAGE && orderedScreenIds.size() > 0) {
            // add by linmq 2018-6-26
//            orderedScreenIds.add(Workspace.CUSTOM_PAGE_SCREEN_POS, Workspace.CUSTOM_PAGE_SCREEN_ID);
        }
        if (FeatureFlags.QSB_ON_FIRST_SCREEN &&
                orderedScreenIds.indexOf(Workspace.FIRST_SCREEN_ID) != 0) {
            if (!FeatureFlags.ADD_CUSTOM_PAGE) { // add by linmq 2018-7-24 切换壁纸时会重新binditems，updateWorkspaceScreenOrder会先clear table导致记录不准确，最后一页item无法显示
     ...
     * Implementation of the method from LauncherModel.Callbacks.
     */
    private boolean mFinishedBindingItems = false; // add bylinmq 2018-7-7 修复尾页横竖屏切换导致setWorkspaceLoading时序问题，桌面无法长按
    public void finishBindingItems() {
        ...
        if (FeatureFlags.ADD_CUSTOM_PAGE) { // add by linmq
            mFinishedBindingItems = true;
        }
